...
exports.uploadZip = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            return res.status(400).json({ error: "Failed to extract zip file. The file may be corrupted or not a valid zip archive." });
        }
        ...
        if (maxDepth > 3) {
            await cleanup([req.file.path, extractPath, categorizedPath]);
            return res.status(400).json({ error: "More than 3 folder levels detected in the zip. Only up to 3 levels are allowed." });
        }
        // Process and organize
        const fileCategories = await fileService.processDirectory(extractPath, 0, 3);
        await fileService.organizeFilesByCategory(fileCategories, categorizedPath);
        // Bundle the categorized result as zip
        const resultZipPath = path.join("categorized", `${sessionId}-result.zip`);
        ...
        res.download(resultZipPath, `${sessionId}-result.zip`, async (err) => {
            ...
            await cleanup([req.file.path, extractPath, categorizedPath, resultZipPath]);
            if (err) {
                console.error("Error sending result zip:", err);
            }
        });
    } catch (error) {
        await cleanup([req.file?.path, extractPath, categorizedPath]);
